...
class TestDBStorage(unittest.TestCase):
    ...
    @unittest.skipIf(models.storage_t != "db", "not testing db storage")
    def test_dbstorage_get_existing_object(self):
        """test that get retrieves one object"""
        new_user = User()
        DBStorage.new(new_user)
        DBStorage.save()

        retrieved_user = DBStorage.get(User, new_user.id)
        self.assertEqual(retrieved_user, new_user)

    @unittest.skipIf(models.storage_t != "db", "not testing db storage")
    def test_dbstorage_get_nonexisting_object(self):
        """test getting non existent object"""
        retrieved_user = DBStorage.get(User, "nonexistent")
        self.assertIsNone(retrieved_user)

    @unittest.skipIf(models.storage_t != "db", "not testing db storage")
    def test_dbstorage_count_specific_class(self):
        """test counting objects of a specific class"""
        count_before = DBStorage.count(User)
        new_user = User()
        DBStorage.new(new_user)
        DBStorage.save()
        count_after = DBStorage.count(User)
